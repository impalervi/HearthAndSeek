"""
scrape_wowdb_quests.py — Scrape quest-sourced decor items from housing.wowdb.com.
...
Usage:
  python scrape_wowdb_quests.py

Output: data/wowdb_quests.json
  Used as seed data by enrich_catalog.py (quest name→ID and NPC→ID mappings).
  Not part of the automated pipeline (run_pipeline.py); run manually when
  you need to refresh quest/NPC seed data before enrichment.

See also:
  scrape_wowdb.py — Scrapes community sets + per-item theme tags (for themed filters).
"""
