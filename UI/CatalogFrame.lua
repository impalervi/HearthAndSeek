...
local FILTER_SECTIONS = {
    ...
    },
    {
        ...
        toggle = "CatalogGrid_ToggleFaction",
        widgetTable = "factions",
        defaultCollapsed = true,
    },
}
...
function SectionBuilders.multiselect(content, sectionDef, panel)
    ...
    for _, key in ipairs(orderList) do
        if key ~= skipKey then
            ...
            elseif wTable == "factions" then
                -- No pre-generated ByFaction table; tally at render time.
                -- Bucket matches the grid predicate (missing = neutral).
                if NS.CatalogData and NS.CatalogData.Items then
                    for _, item in pairs(NS.CatalogData.Items) do
                        if NS.GetFactionBucket(item) == key then count = count + 1 end
                    end
                end
            end
...
local FILTER_BAR_BUTTONS = {
    ...
    { key = "factions",   label = "Faction",     sectionIDs = { "factions" },   width = 200 },
